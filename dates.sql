...
Важно!!!
Перед преобразованием строк в даты, убедиться, что в столбце нет 
строк типа "-". Эти строки надо заменить на NULL:
...
PS Можно также при замене использовать конструкцию CASE


1. Если в строке даты соблюден порядок год-месяц-день:

SELECT date('2012/03/19') AS date_converted
SELECT date('2012.03.19') AS date_converted
...
SELECT to_date('19.03.2012', 'DD.MM.YYYY') AS date_converted
...
3. Пример совмещения удаления ненужных строк и преобразования строки
Если в ячейке символ "-", то ставится NULL, 
в противном случае строка преобразуется в дату

SELECT
	date(NULLIF(date_first_observed, '-')) AS date_first_observed
...
SELECT to_char('2012-03-19'::date, 'DD.MM.YYYY') AS date_converted
...
Очень похоже на преобразование строки в дату, но добавляются особенности формата.

SELECT now()

SELECT to_timestamp('2012-03-19 02:23:', 'YY-MM-DD HH24:MI') 

SELECT to_timestamp('2012-03-19 02:23:', 'YY-MM-DD HH24:MI')::TIME
/*
to_timestamp|
------------+
    02:23:00|
...
----------------------------------------------------------------------------------
-- Преобразование строки или TIMESTAMP в TIME
----------------------------------------------------------------------------------

SELECT '2012-03-19 02:23:'::TIME
